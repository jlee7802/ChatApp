...
class ChatRoom extends React.Component {
	...
	sendMessage = (event) => {
		...
		if(code === 13) {
    		this.state.stompClient.send("/app/hello", {}, JSON.stringify({'name': this.props.userId, 'content': this.state.inputValue, 'chatId': this.props.chatId})); // Use this.props.userName for name
    		this.setState({inputValue: ''});
    	}
	}

	render(){
		console.log("the chat id is: " + this.props.chatId);
